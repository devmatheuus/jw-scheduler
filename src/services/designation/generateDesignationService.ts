import { Any, In, IsNull, Like, Not } from 'typeorm';
import { appDataSource } from '../../dataSource';
import {
  CreateChristian,
  GetChristian,
} from '../../types/christian/createChristian';
...
export const generateDesignationService = async (period: string) => {
  ...
  const specificMeetingData = returnSpecificMeetingData(period);
      ...
      where: {
        ...
        lastPersonParticipatedName: Not(
          Like(
            `%${
              publishers.length > 0
                ? publishers[publishers.length - 1]?.lastPersonParticipatedName
                : 'not found'
            }%`
          )
        ),
  ...
  for (let index = 0; index < parts.length; index++) {
    ...
    const designation = {
      ...
      assistant: {
        ...
        lastPersonParticipatedName: currentPublisher.lastPersonParticipatedName,
      },
    };
